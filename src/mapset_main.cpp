...
string to_lowercase(const string& str) {
    ...
    return string(lower_view.begin(), lower_view.end());
}
...
MapArray<string, int> count_words(istream& document, SetList<string>& stopwords) {
    auto words_view = ranges::istream_view<string>(document) | views::transform(to_lowercase) | views::filter([&](const string& s) { return !stopwords.contains(s); });
...
void output_word_counts(MapArray<string, int>& word_counts, ostream& out) {
